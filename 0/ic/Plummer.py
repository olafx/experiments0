...
i = 0
while i < n:
    r = np.random.uniform(0, r_max, n_batch)
    v = np.random.uniform(0, np.sqrt(2*n/np.sqrt(r_max**2+a**2)), n_batch)
    p = np.random.uniform(0, 24*np.sqrt(2)/(7*np.pi**3)/(n*a)**1.5, n_batch)
...
r_theta = np.arccos(np.random.uniform(-1, 1, n))
v_theta = np.arccos(np.random.uniform(-1, 1, n))
r_phi = np.random.uniform(0, 2*np.pi, n)
